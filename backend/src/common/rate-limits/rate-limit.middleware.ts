...
export class RateLimitMiddleware implements NestMiddleware {
  ...
  constructor(
    ...
    if (redisUrl) {
      ...
      this.redis.on('error', (error) => {
        console.warn('[RateLimit] Redis unavailable; limiter will fail open for this request.', error.message);
      });
    }
  }
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      const applicableRules = rules.filter((rule) => this.hasIdentityForScope(identity, rule.scope));
      ...
      for (const rule of applicableRules) {
        ...
        try {
          ...
          consumed.push({ rule, result });
        } catch (error) {
          if (error instanceof RateLimiterRes || this.isRateLimiterRes(error)) {
            this.sendRateLimitExceeded(res, rule, error as RateLimiterRes);
            return;
          }
      ...
      console.warn('[RateLimit] Failed to evaluate request; allowing request to continue.', error);
      next();
    }
  }
  ...
  private async resolveIdentity(req: Request, apiKey?: string): Promise<RateLimitIdentity> {
    const userId = this.extractUserId(req);
    const requestedCompanyId = this.getHeader(req, 'x-company-id') || this.extractCompanyIdFromPath(req.path);
    const companyId = requestedCompanyId || (userId ? await this.resolveActiveCompanyId(userId) : undefined);
  ...
  private extractUserId(req: Request) {
    ...
    try {
      const payload = jwt.verify(token, this.jwtSecret) as { sub?: string; userId?: string };
  ...
  private hasIdentityForScope(identity: RateLimitIdentity, scope: RateLimitScope) {
  ...
  private mostConstrainedLimit(consumed: ConsumedLimit[]) {
    return consumed.sort((a, b) => a.result.remainingPoints - b.result.remainingPoints)[0];
  }
  ...
  private setRateLimitHeaders(res: Response, consumed?: ConsumedLimit) {
    ...
    res.setHeader('X-RateLimit-Remaining', String(Math.max(consumed.result.remainingPoints, 0)));
    res.setHeader('X-RateLimit-Reset', String(resetAt));
  }

  private sendRateLimitExceeded(res: Response, rule: RateLimitRule, limiterResult: RateLimiterRes) {
    const retryAfterSeconds = Math.max(Math.ceil(limiterResult.msBeforeNext / 1000), 1);
