...
import { useRealtimeAttendanceStore } from "@/store/useRealtimeAttendanceStore";
import {
  usePayrollRunStore,
  type ProcessingStep,
} from "@/store/usePayrollRunStore";
import {
...
export const useWebSocketEvents = () => {
  ...
  const { addNotification, syncUnreadCount } = useNotificationStore();
  const {
    setPayrollStatus,
    incrementEmployeeDelta,
    incrementNotificationDelta,
  } = useDashboardRealtimeStore();
  const setComplianceAlerts = usePlatformStore((s) => s.setComplianceAlerts);
  const { updateClockIn, updateClockOut } = useRealtimeAttendanceStore();

  const showRealtimeNotification = (
    notification: ReturnType<typeof addNotification>,
  ) => {
    if (
    ...
    const onConnect = async () => {
      ...
      try {
        const res = await fetch(
          `/api/events?since=${encodeURIComponent(lastDisconnectIso)}`,
        );
        if (res.ok) {
  ...
  useEffect(() => {
    if (!socket) return;
    // Payroll Events
    const handlePayrollRunStatusUpdate = (data: {
      runId: string;
      status: string;
      progress: number;
    }) => {
      ...
      queryClient.setQueriesData(
        ...
        (oldData: any) => {
          ...
          const patchRun = (run: any) =>
            run?.id === data.runId
              ? { ...run, status: data.status, progress: data.progress }
              : run;
          if (Array.isArray(oldData)) return oldData.map(patchRun);
          if (Array.isArray(oldData.data))
            return { ...oldData, data: oldData.data.map(patchRun) };
          return oldData;
        },
      );
            ...
            : data.progress >= 75
              ...
              : data.progress >= 50
                ...
                : data.progress >= 25
                  ? "submitting_ach"
                  : "calculating";
        const payrollRunStore = usePayrollRunStore.getState();
        payrollRunStore.setRunState(
          data.progress >= 100 ? "complete" : "processing",
        );
        payrollRunStore.setProcessingStep(step);
        payrollRunStore.setShowProcessing(data.progress < 100);
      }
    };

    const handlePayrollStatusChanged = (data: {
      status: string;
      employeeCount?: number;
    }) => {
      const normalizedStatus = data.status.toLowerCase();
      setPayrollStatus({
        isRunning: ["processing", "running", "in_progress"].includes(
          normalizedStatus,
        ),
    ...
    const handlePayrollRunCompleted = (data: {
      runId: string;
      totalGross: number;
      employeeCount: number;
    }) => {
      ...
      if (window.location.pathname.includes("/payroll/run")) {
        const payrollRunStore = usePayrollRunStore.getState();
        payrollRunStore.setRunState("complete");
        payrollRunStore.setProcessingStep("complete");
        payrollRunStore.setShowProcessing(false);
      }
      queryClient.invalidateQueries({ queryKey: ["payroll-runs"] });
      toast.success(
        `Payroll run completed: $${data.totalGross.toLocaleString()} for ${data.employeeCount} employees`,
      );
    };

    const handlePayrollApprovalRequired = (data: {
      runId: string;
      approverIds: string[];
    }) => {
      queryClient.invalidateQueries({ queryKey: ["payroll-runs"] });
    ...
    const handleEmployeeTerminated = (data: {
      employeeId: string;
      lastDay: string;
    }) => {
      queryClient.invalidateQueries({ queryKey: ["employees"] });
    ...
    const handleEmployeeTerminationInitiated = (data: {
      id: number;
      ...
      queryClient.invalidateQueries({ queryKey: ["employees"] });
      queryClient.invalidateQueries({
        queryKey: ["employee", String(data.id)],
      });
      queryClient.invalidateQueries({ queryKey: ["payroll-runs"] });
      queryClient.invalidateQueries({ queryKey: ["pto-requests"] });
      toast.warning(
        `Termination initiated — final pay due ${data.finalPayDate ?? "next scheduled pay date"}`,
      );
    };

    const handleEmployeeAccessRevoked = (data: {
      id: number;
      terminationDate: string;
    }) => {
      queryClient.invalidateQueries({
    ...
    const handleEmployeeCobraTriggered = (data: {
      id: number;
      terminationDate: string;
    }) => {
      ...
      toast.info(
        "COBRA eligibility event triggered — notice required within 14 days",
      );
    };

    const handleEmployeeClockedIn = (data: {
      employeeId: string;
      timestamp: string;
    }) => {
      updateClockIn(data);
    ...
    const handleEmployeeClockedOut = (data: {
      employeeId: string;
      timestamp: string;
      hoursWorked: number;
    }) => {
    ...
    // Time Events
    const handleTimePtoRequested = (data: {
      requestId: string;
      employeeId: string;
      dates: string[];
    }) => {
    ...
    const handleTimeOvertimeAlert = (data: {
      employeeId: string;
      hoursThisWeek: number;
    }) => {
      toast.warning(
        `Overtime alert: Employee has worked ${data.hoursThisWeek} hours this week`,
      );
    };

    // Hiring Events
    const handleAtsCandidateApplied = (data: {
      jobId: string;
      candidateId: string;
    ...
    const handleAtsCandidateStageChanged = (data: {
      candidateId: string;
      fromStage: string;
      toStage: string;
    }) => {
      queryClient.invalidateQueries({ queryKey: ["candidates"] });
      queryClient.invalidateQueries({
        queryKey: ["candidate", data.candidateId],
      });
    };

    const handleAtsOfferSigned = (data: {
      candidateId: string;
      jobId: string;
    }) => {
      queryClient.invalidateQueries({ queryKey: ["candidates"] });
    ...
    const handleEmployeeAutoCreatedFromAts = (data: {
      employeeId: number;
      candidateId: number;
    ...
    const handleNotificationNew = (data: {
      ...
      const notification = data.notification ?? {
        ...
        message:
          data.description ?? "A new dashboard notification was received.",
      };
    ...
    const handleWorkflowActionExecuted = (data: {
      ...
      if (data.entityType === "employee") {
        queryClient.invalidateQueries({
          queryKey: ["employees", String(data.entityId)],
        });
        queryClient.invalidateQueries({ queryKey: ["employees"] });
      }
      ...
      if (data.actionType === "change_status") {
        queryClient.invalidateQueries({
          queryKey: ["dashboard", data.entityType],
        });
      }
      ...
      queryClient.setQueryData(["dashboard", "admin"], (oldData: any) => {
        const nextEvent = {
          ...
          avatarSeed: "System",
          action: firstField
            ? `automatically updated ${firstField}`
            : `performed ${data.actionType.replace(/_/g, " ")}`,
        ...
        return {
          ...
          activityFeed: [
            nextEvent,
            ...(oldData?.activityFeed || []).slice(0, 19),
          ],
        };
      });
    ...
    // Compliance Events
    const handleComplianceAlertNew = (data: {
      alertId: string;
      severity: string;
      description: string;
    }) => {
      if (data.severity === "critical") {
        setComplianceAlerts({ critical: 1, warning: 0 });
        // Show orange topbar banner
        toast.error(`Critical compliance alert: ${data.description}`, {
          duration: 10000, // Show for 10 seconds
        });
      } else {
        setComplianceAlerts({ critical: 0, warning: 1 });
        toast.warning(`Compliance alert: ${data.description}`);
      }
    };

    // System Events
    const handleSystemMaintenanceScheduled = (data: {
      startAt: string;
      duration: number;
    }) => {
      toast.warning(
        `System maintenance scheduled for ${new Date(data.startAt).toLocaleString()}`,
      );
    };

    const handleFeatureAnnouncement = (data: {
      title: string;
      description: string;
      ctaUrl?: string;
    }) => {
    ...
    const handleAnnouncementPublished = (data: {
      ...
      toast.info(data.title, {
        description:
          data.priority === "Urgent"
            ...
            : "New company announcement published.",
      });
    };
    ...
    const typedNotificationHandlers = notificationDefinitions.map(
      (definition) => {
        const handler = (data: any) =>
          handleTypedNotification(definition.type, data);
    ...
    return () => {
      ...
      typedNotificationHandlers.forEach(({ event, handler }) =>
        off(event, handler),
      );
