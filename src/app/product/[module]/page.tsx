...
export async function generateMetadata({
  ...
  const selected = MODULE_DATA[module as ModuleSlug];

  if (!selected) {
    return {
      title: "CircleWorks Product",
      description: "Payroll and HR built for U.S. companies.",
    };
  }

  const title = `${selected.name} Software | CircleWorks`;
  const description = selected.hero.stat;
...
export default async function ModulePage({
  ...
  const accent =
    accentClasses[mod.accent as keyof typeof accentClasses] ||
    accentClasses.blue;
  ...
  return (
    <main className="min-h-screen bg-white font-sans selection:bg-blue-200 selection:text-slate-950">
      ...
      <section
        className={
          "relative pt-32 pb-20 lg:pt-48 lg:pb-32 bg-[#0A1628] overflow-hidden"
        }
      >
        <div
          className={`absolute top-0 right-0 w-[600px] h-[600px] ${accent.bg} rounded-full blur-[150px] mix-blend-screen opacity-20`}
        />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex flex-col lg:flex-row items-center gap-16">
            <div className="flex-1 text-center lg:text-left">
              <span
                className={`text-sm uppercase tracking-widest font-bold mb-4 block ${accent.text}`}
              >
              ...
              <p className="text-xl text-slate-300 font-medium mb-8 max-w-2xl mx-auto lg:mx-0">
                {mod.hero.stat}
              </p>
              <div className="mb-10 grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-2xl mx-auto lg:mx-0">
                {mod.hero.mockupStats.map((stat) => (
                  <div
                    key={stat}
                    className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-bold text-white shadow-sm"
                  >
                    {stat}
                  </div>
                ))}
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
                <Link
                  href="#trial"
                  className={`px-8 py-4 rounded-full bg-gradient-to-r ${accent.gradFrom} ${accent.gradTo} text-white font-bold text-[16px] hover:shadow-[0_0_20px_rgba(255,255,255,0.2)] transition-shadow`}
                >
                  Start with {mod.name} Free
                </Link>
                <Link
                  href="#how-it-works"
                  className="px-8 py-4 rounded-full text-white font-bold text-[16px] hover:bg-white/10 transition-colors"
                >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h3 className="text-2xl font-bold text-slate-900 tracking-tight">
              Trusted by 5,000+ HR Teams using {mod.name}
            </h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {mod.testimonials.map((test, i) => (
              <div
                key={i}
                className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm flex flex-col hover:shadow-md transition-shadow"
              >
                <div className="flex text-amber-400 mb-4">{"★".repeat(5)}</div>
                <p className="text-slate-700 font-medium mb-6 flex-1 italic">
                  "{test.quote}"
                </p>
                <div className="flex items-center gap-3">
                  <div
                    className={`w-10 h-10 rounded-full ${accent.bg} text-white font-bold flex items-center justify-center text-sm`}
                  >
                    {test.author[0]}
                  </div>
                  <div>
      ...
      <section id="how-it-works" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span
              className={`text-sm uppercase tracking-widest font-bold mb-3 block ${accent.text}`}
            >
              Workflow
            </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            ...
            {mod.howItWorks.map((step, i) => (
              ...
              >
                <div
                  className={`w-20 h-20 rounded-full flex items-center justify-center text-2xl font-black text-white ${accent.bg} border-8 border-white shadow-xl mb-6`}
                >
                  {i + 1}
                </div>
      ...
      {mod.complianceNote && (
        <section className="py-20 bg-white">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div
              className={`rounded-3xl p-10 border-2 ${accent.border} ${accent.lightBg} relative overflow-hidden shadow-sm`}
            >
              <ShieldCheck
                size={120}
                className={`absolute -right-10 -bottom-10 opacity-10 ${accent.text}`}
              />
              <h3
                className={`text-2xl font-black ${accent.text} mb-4 flex items-center gap-3`}
              >
      ...
      <section className="py-24 bg-[#0A1628] text-white overflow-hidden relative">
        <div
          className={`absolute top-0 right-0 w-[400px] h-[400px] ${accent.bg} rounded-full blur-[120px] mix-blend-screen opacity-20`}
        />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <p className="text-xl text-slate-400 max-w-2xl mx-auto mb-12">
            Seamlessly connects your {mod.name} workflows via two-way sync.
          </p>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-6">
            {mod.faqs.map((faq, i) => (
              <details
                key={i}
                className={`group border border-slate-200 rounded-2xl overflow-hidden [&_summary::-webkit-details-marker]:hidden ${accent.lightBg} bg-opacity-30`}
              >
      ...
      <section className="py-32 bg-slate-50 relative overflow-hidden border-t border-slate-200">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <Link
            href="/signup"
            className={`inline-block px-12 py-5 rounded-full bg-gradient-to-r ${accent.gradFrom} ${accent.gradTo} text-white font-bold text-xl hover:scale-105 transition-transform shadow-[0_10px_40px_rgba(0,0,0,0.15)]`}
          >
            Start with {mod.name} Free
          </Link>
        </div>
