...
type PortalDocument = (typeof mockEmployeeDocuments)[number] & {
  fileUrl?: string | null;
};
...
export default function DocumentsPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/employees/me/documents")
      .then((res) => (res.ok ? res.json() : { documents: [] }))
      .then((payload) => {
        if (isMounted) {
          setEmployeeDocs(payload.documents || []);
        }
      })
      .catch(() => {
        if (isMounted) {
  ...
  const personalDocs = allDocs.filter(
    (d) =>
      d.type === "Personal" || d.type === "Pay Stub" || d.type === "Tax Form",
  );
  const pendingSign = allDocs.filter((d) => d.status === "Pending Signature");
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            View, sign, and manage your documents
          </p>
        </div>
        <button
          onClick={() => toast("File upload modal coming soon")}
        ...
        <div className="rounded-xl border-2 border-amber-200 dark:border-amber-800/40 bg-amber-50 dark:bg-amber-900/10 p-4">
          ...
          {pendingSign.map((doc) => (
            <div key={doc.id} className="flex items-center gap-3 mt-2">
              ...
              />
              <span className="text-[13px] font-semibold text-amber-900 dark:text-amber-200 flex-1">
                {doc.name}
              </span>
              <button
                onClick={() => toast.success(`Opened ${doc.name} for signing`)}
                className="text-[12px] font-bold text-amber-700 dark:text-amber-400 hover:underline"
              >
                Sign Now →
              </button>
            </div>
      ...
      <div className="flex items-center gap-1 p-1 rounded-lg bg-slate-100 dark:bg-slate-800 w-fit">
        {(["company", "personal"] as const).map((tab) => (
          <button
            ...
            className={`px-4 py-2 rounded-md text-[13px] font-bold transition-all ${activeTab === tab ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm" : "text-slate-500 dark:text-slate-400"}`}
          >
        ...
        {docs.map((doc, i) => {
          ...
          return (
            ...
            >
              ...
              {statusInfo && (
                <span
                  className={`px-2 py-1 rounded-full text-[10px] font-bold flex items-center gap-1 flex-shrink-0 ${statusInfo.bg} ${statusInfo.text}`}
                >
              ...
              <button
                ...
                className="p-2 rounded-lg hover:bg-violet-50 dark:hover:bg-violet-900/20 text-slate-400 hover:text-violet-600 dark:hover:text-violet-400 transition-colors flex-shrink-0"
              >
                <Download size={16} />
              </button>
            </motion.div>
