...
import { BAN_THE_BOX_JURISDICTIONS } from "@/utils/compliance";
                  ...
                  <div className="flex flex-col gap-4">
                     <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                           {BAN_THE_BOX_JURISDICTIONS.map(j => (
                              <div key={j.id} className="flex items-center justify-between rounded-lg border border-blue-100 bg-blue-50 px-3 py-2 text-xs font-bold text-blue-800 dark:border-blue-800/50 dark:bg-blue-900/20 dark:text-blue-300">
                                 <span>{j.label}</span>
                                 <span className="rounded bg-white/80 px-2 py-0.5 text-[10px] text-blue-600 dark:bg-slate-900/60 dark:text-blue-300">{j.stateCode}</span>
                              </div>
                           ))}
                        </div>
                        <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400">
                           {BAN_THE_BOX_JURISDICTIONS.length} active jurisdictions. Annual update scheduled every Q4 before new state notices take effect.
                        </div>
                     </div>
                  </div>
               </div>
