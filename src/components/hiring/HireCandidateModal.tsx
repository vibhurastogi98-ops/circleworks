...
import { AlertTriangle, CheckCircle2, User, Mail, Phone, Calendar, Briefcase, DollarSign, Loader2 } from "lucide-react";
...
export function HireCandidateModal({ isOpen, onClose, candidate, offer }: HireCandidateModalProps) {
  ...
  const hasStartDate = !!offer.startDate;

  const isBlocked = !hasName || !hasEmail;
  const hasWarnings = !hasPhone || !hasStartDate;
  ...
  const handleHire = async (ignoreDuplicate = false) => {
    ...
    try {
      ...
      toast.success(`Employee created and pre-boarding invitation sent to ${candidate.email}`);
      onClose();
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Hire Candidate: {candidate.firstName} {candidate.lastName}
          </DialogTitle>
          <DialogDescription>
            Confirm final details before converting this applicant to an employee record.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 my-4">
          ...
          <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 overflow-hidden">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Pre-flight Checklist</h4>
          ...
          {errorStatus === "CONFLICT" && (
            <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 p-4 rounded-xl flex gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-amber-700 dark:text-amber-500 mt-1">An employee with the email <strong>{candidate.email}</strong> already exists. Would you like to merge or create a new record anyway?</p>
